...
$("#satellite_sites").multiselect().multiselectfilter();

<?php
   foreach($satellites as $satellite){
 ?>
   $("select#satellite_sites").multiselect("widget").find(":checkbox[value='" + <?php echo $satellite['id']; ?> + "']").each(function() {
	   $(this).click();
   });
<?php
   }
...
function processSatellites(){
    var satellites = $("select#satellite_sites").multiselect("getChecked").map(function() {
		return this.value;
	}).get();	
	$("#satellite_sites_holder").val(satellites);	
}	
	  ...
	  <div class="row-fluid row">
	    ...
	    <div class="span12 span-fixed-sidebar" >
	      	<div class="hero-unit" style="padding-bottom:20px;background: rgb(184, 255, 184);">
				...
				<?php
					$attributes = array('class' => 'input_form','onsubmit'=>'return processSatellites()');
						...
						<fieldset>
							...
							<table class="facility_basic_info" style="width:70%;">
							   ...
							   <tr><td><strong class="label">County</strong></td>
								...
								<tr><td><strong class="label">Satellite Site(s)</strong></td>
									<td><select class="input-xlarge" id="satellite_sites" name="satellite_sites[]" multiple="multiple" style="width:400px;">
											<?php foreach($sites as $site){
												if($site['name'] !=""){
												?>
										...
										<input type="hidden" id="satellite_sites_holder" name="satellite_sites_holder" />
									</td>
								</tr>
