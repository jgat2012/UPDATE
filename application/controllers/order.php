...
class Order extends MY_Controller {
	...
	public function authenticate_user($login_type = 0) {
		...
		if ($login_type == 1) {
			...
			$url = $this -> nascop_url . 'sync/user/' . $email_address;
		}
	...
	public function generate_table($columns, $data = array(), $table = "cdrr") {
		...
		foreach ($data as $mydata) {
			...
			if ($status_name == "prepared") {
				$links = array("order/view_order/" . $table => "view", "order/update_order/" . $table => "update", "order/delete_order/" . $table => "delete", "order/download_order/" . $table => "download");
			} else {
	...
	public function save($type = "cdrr", $status = "prepared", $id = "") {
		...
		if ($type == "maps") {
			...
			if ($save) {
				...
				$total_adult = $this -> input -> post("total_cotr_adult");
				$total_child = $this -> input -> post("total_cotr_child");
				...
				if ($regimens != null) {
					...
					foreach ($regimens as $regimen) {
						...
						//echo $maps_item[$regimen_counter]['total'] .":".$maps_item[$regimen_counter]['regimen_id']."<br>";
						$regimen_counter++;
		...
		if ($status == "approved") {//Upload data to Nascop
			//Clean ids,cdrr_ids and map_ids
			$main_array = $this -> clean_index($type, $main_array);
			$main_array = array($main_array);
			...
			if ($supplier != "KEMSA") {
				...
				if (is_array($responses)) {
					if (!empty($responses)) {
						...
						/*
						 * Go to nascop to save new escm order but and save in local
						 *
						 $target_url = "sync/save/escm/" . $type;
						 $url = $this -> nascop_url . $target_url;
						 $json_data = json_encode($responses);

						 $responses = $this -> post_order($url, $json_data, "KEMSA");
						 */
					}
				}
			} else {

				//Go to nascop to save new order
			...
			$this -> session -> set_flashdata('order_message', "Your " . strtoupper($type) . " data was successfully approved !");
			...
			if ($supplier != "KEMSA") {
				...
				$this -> put_order($url, $json_data);
				$responses = json_decode($json_data, TRUE);
				$id = $this -> extract_order($type, $responses, $id);
			}
			//update nascop
			$target_url = "sync/save/escm/" . $type . "/" . $id;
			$url = $this -> nascop_url . $target_url;
			$json_data = json_encode($responses);
			$responses = $this -> post_order($url, $json_data, "KEMSA");
			$this -> session -> set_flashdata('order_message', "Your " . strtoupper($type) . " data was successfully reviewed !");
			...
			$this -> session -> set_flashdata('order_message', "Your " . strtoupper($type) . " data was successfully updated !");
			redirect("order/update_order/" . $type . "/" . $id);
		}
	}
	...
	public function download_order($type = "cdrr", $id) {
		...
		if ($type == "cdrr") {
			...
			if ($cdrr_array[0]['code'] == 'D-CDRR') {
				...
				foreach ($logs as $log) {
					if ($log -> description == "prepared") {
						...
						$objPHPExcel -> getActiveSheet() -> SetCellValue('C113', $log -> user -> Phone_Number);
						$objPHPExcel -> getActiveSheet() -> SetCellValue('K111', $log -> user -> Access -> Level_Name);
						...
						$objPHPExcel -> getActiveSheet() -> SetCellValue('C117', $log -> user -> Phone_Number);
						$objPHPExcel -> getActiveSheet() -> SetCellValue('K115', $log -> user -> Access -> Level_Name);
				...
				foreach ($logs as $log) {
					if ($log -> description == "prepared") {
						...
						$objPHPExcel -> getActiveSheet() -> SetCellValue('C109', $log -> user -> Phone_Number);
						$objPHPExcel -> getActiveSheet() -> SetCellValue('K107', $log -> user -> Access -> Level_Name);
			...
			$status_name = strtolower($fmaps_array[0]['status_name']);
			if ($status_name != 'prepared') {
				$sql = "SELECT m.created as map_created,ml.created as log_created,m.id as map_id,u.*,al.level_name
					FROM maps m
					LEFT JOIN maps_log ml ON ml.maps_id=m.id
					LEFT JOIN users u ON u.map=ml.user_id
					LEFT JOIN access_level al ON al.id=u.Access_Level
					WHERE m.id=" . $fmaps_id;
				$query = $this -> db -> query($sql);
				$user_prep = $query -> result_array();
				if ($report_type == "D-CDRR") {
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B142', @$user_prep[0]['Name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B144', @$user_prep[0]['Phone_Number']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('G142', @$user_prep[0]['level_name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('E144', @$user_prep[0]['map_created']);

					$objPHPExcel -> getActiveSheet() -> SetCellValue('B146', $fmaps_array[0]['Name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B148', $fmaps_array[0]['Phone_Number']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('G146', $fmaps_array[0]['level_name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('E148', $fmaps_array[0]['created']);
				} else {
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B177', @$user_prep[0]['Name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B179', @$user_prep[0]['Phone_Number']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('G177', @$user_prep[0]['level_name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('E179', @$user_prep[0]['map_created']);

					$objPHPExcel -> getActiveSheet() -> SetCellValue('B181', $fmaps_array[0]['Name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B183', $fmaps_array[0]['Phone_Number']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('G181', $fmaps_array[0]['level_name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('E183', $fmaps_array[0]['created']);
				}

			} else {
				if ($report_type == "D-MAPS") {
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B142', $fmaps_array[0]['Name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B144', $fmaps_array[0]['Phone_Number']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('G142', $fmaps_array[0]['level_name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('E144', $fmaps_array[0]['created']);
				} else {
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B177', $fmaps_array[0]['Name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('B179', $fmaps_array[0]['Phone_Number']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('G177', $fmaps_array[0]['level_name']);
					$objPHPExcel -> getActiveSheet() -> SetCellValue('E179', $fmaps_array[0]['created']);
				}
			}

			//Generate file
			ob_start();
			$original_filename = $fmaps_array[0]['maps_id'] . " " . $fmaps_array[0]['facility_name'] . " " . $fmaps_array[0]['period_begin'] . " to " . $fmaps_array[0]['period_end'] . ".xls";
			$filename = $dir . "/" . urldecode($original_filename);
			$objWriter = new PHPExcel_Writer_Excel5($objPHPExcel);
			$objWriter -> save($filename);
			$objPHPExcel -> disconnectWorksheets();
	...
	public function getPeriodRegimenPatients($from, $to) {
		...
		$sql = "SELECT count(*) as patients, r.regimen_desc,r.regimen_code,$regimen_column as regimen 
		        FROM patient p
	...
	public function getCentralDataMaps($start_date, $end_date,$data_type ='') {//Get data when generating reports for central site
		...
		if (isset($facility_code)) {
			...
			if($data_type=='art'){
				...
				$date = date('Y-m-d');
				$sql_art = '
						SELECT IF(round(datediff(CURDATE(),p.dob)/360)>15,"art_adult","art_child") as age,COUNT(p.id) as total FROM patient p 
				...
				//New , only get ART, OI, PMTCT pregnant women
				$sql_clients = 'SELECT COUNT(p.id) as total,IF(p.gender=1,"new_male","new_female") as gender FROM patient p
								LEFT JOIN patient_status ps ON ps.id=p.current_status
								LEFT JOIN regimen_service_type rs ON rs.id=p.service
								LEFT JOIN regimen r ON r.id = p.current_regimen
				...
				//revisit
				$sql_clients = 'SELECT COUNT(p.id) as total,IF(p.gender=1,"revisit_male","revisit_female") as gender 
								FROM patient p
								LEFT JOIN patient_status ps ON ps.id=p.current_status
								LEFT JOIN regimen_service_type rs ON rs.id=p.service
				...
				$sql_clients = 'SELECT COUNT(p.id) as total
							  FROM patient p
							  LEFT JOIN regimen r ON r.id = p.current_regimen
							  LEFT JOIN regimen_category rc ON rc.id = r.category
							  LEFT JOIN patient_status ps ON ps.id=p.current_status
				...
				//New
				$sql_clients = 'SELECT COUNT(p.id) as total FROM patient p
							  LEFT JOIN regimen r ON r.id = p.current_regimen
							  LEFT JOIN regimen_category rc ON rc.id = r.category
							  LEFT JOIN patient_status ps ON ps.id=p.current_status
				...
				$sql_clients = 'SELECT IF(round(datediff(CURDATE(),p.dob)/360)>15,"pep_adult","pep_child") as age,COUNT(p.id) as total FROM patient p 
							LEFT JOIN regimen_service_type rs ON rs.id=p.service
							LEFT JOIN patient_status ps ON ps.id=p.current_status
				...
				$sql_clients = 'SELECT IF(round(datediff(CURDATE(),p.dob)/360)>15,"total_adult","total_child") as age,COUNT(p.id) as total
								FROM  patient p 
				...
				//New
				$sql_clients = "
								SELECT IF(oi.name LIKE '%cryptococcal%','new_cm',
							    	   IF(oi.name LIKE '%oesophageal%','new_oc','')) as OI, COUNT(DISTINCT(p.patient_number_ccc)) as total 
							    	   FROM patient p
								INNER JOIN patient_visit pv ON pv.patient_id = p.patient_number_ccc
								INNER JOIN opportunistic_infection oi ON oi.indication = pv.indication
								INNER JOIN patient_status ps ON ps.id=p.current_status
								WHERE (oi.name LIKE '%cryptococcal%' OR oi.name LIKE '%oesophageal%')
				...
				//Revisit
				$sql_clients="SELECT IF(temp.OI LIKE '%meningitis%','revisit_cm','revisit_oc') as OI,COUNT(temp.patient_number_ccc) as total
							  FROM(
								  	SELECT pv.patient_number_ccc,pv.current_status,pv.date_enrolled,oi.name AS OI
									FROM v_patient_visits pv
	...
	public function aggregate_download($period_begin,$facility_id,$cdrr_id,$fmaps_id,$facility_code) {
		...
		//set worksheet index for D-CDRR
		...
		//set worksheet index for D-MAPS
		...
		//Generate file
		ob_start();
		$file = "AGGR#" . date('Ym', strtotime($period_begin)) . ".xls";
	...
	public function getOtherTransactions($param=array(),$row=array()){
		...
		if ($row['physical_stock'] <= 0) {
			...
			if ($results) {
				...
				$row['stock_out'] = date('t', strtotime($period_begin));
			}
		} else {
			$row['stock_out'] = 0;
		}
