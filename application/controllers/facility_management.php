...
class Facility_Management extends MY_Controller {
	...
	public function update() {
		...
		if ($facility_id) {
			$data = array('facilitycode' => $this -> input -> post('facility_cod'), 'name' => $this -> input -> post('facility_name'), 'adult_age' => $this -> input -> post('adult_age'), 'facilitytype' => $this -> input -> post('facility_type'), 'district' => $this -> input -> post('district'), 'county' => $this -> input -> post('county'), 'weekday_max' => $this -> input -> post('weekday_max'), 'weekend_max' => $this -> input -> post('weekend_max'), 'supported_by' => $this -> input -> post('supported_by'),'phone' => $this -> input -> post('phone_number'), 'service_art' => $art_service, 'service_pmtct' => $pmtct_service, 'service_pep' => $pep_service, 'supplied_by' => $this -> input -> post('supplied_by'), 'parent' => $this -> input -> post('central_site'), );
		...
		//clear satellites
		$facility_code = $this -> input -> post('facility_cod');
		$sql = "UPDATE facilities SET parent='' WHERE facilitycode !=parent AND parent='$facility_code'";
		$this -> db -> query($sql);

		//save satellites
		$satellites = $this -> input -> post("satellite_sites_holder");
		if ($satellites != null) {
			$satellites = explode(",", $satellites);
			foreach ($satellites as $satellite) {
				$this -> db -> where('id', $satellite);
				$this -> db -> update('facilities', array("parent" => $facility_code));
			}
		}
